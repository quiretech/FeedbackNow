...
modified: '2026-01-27T21:18:35.070Z'
---
...
| Check-in | Staff | Staff Mode → Button 0 → NFC tap | EPD: "Cleaning in progress", uplink |
| Check-out | Staff | Staff Mode → Button 1 → NFC tap | EPD: "Last Cleaned: [time]", uplink |
| Registered Vote | Staff | Staff Mode → Button 2-5 → NFC tap | Uplink with button ID + NFC ID |
...
- LoRaWAN Class A
- OTAA activation
- US915 frequency plan (configurable)
...
- **Variant A:** Deployments requiring visible "Last Cleaned" timestamp (bathrooms, public facilities)
- **Variant B:** Simpler feedback-only deployments, lower cost, or pre-existing signage
...
| Component Failed | Device Behavior |
|------------------|-----------------|
| SDHC card missing | Full operation, no local logging |
| EPD failure | LED-only feedback, uplinks continue |
| NFC failure | Public mode works, Staff Mode unavailable |
| RTC invalid | Uses network time after join, timestamps may be approximate |
...
| # | Item | Options | Recommendation | Impact |
|---|------|---------|----------------|--------|
| C1 | **"Cleaning in progress" button blocking** | A) Block public buttons during cleaning<br>B) Allow votes during cleaning | B) Allow votes | UX: Prevents missed feedback vs. simpler logic |
| C2 | **Custodian no-checkout timeout** | A) EPD stays "Cleaning in progress" indefinitely<br>B) Auto-revert after X hours<br>C) Auto-revert to previous timestamp<br>D) Auto-revert to current time | A) Stay indefinitely (backend handles) | Stateless device; backend can alert on missing checkout |
| C3 | **Downlink EPD latency** | Once-daily heartbeat = up to 24hr delay for EPD updates | Acceptable for Class A battery device | Customer expectation alignment |

**Note:** These items do not block development but should be confirmed before deployment.
...
| A2 | Button colors | 0=Green, 1=Yellow, 2-5=Red |
| A3 | Staff Mode entry | Button 0+1 hold 2 seconds |
| A4 | Device Info entry | Button 0+1+5 hold 3 seconds |
| A5 | Reboot trigger | All 6 buttons hold 10 seconds |
| A6 | NFC data format | 4-byte NDEF user record (not 8-byte UID) |
| A7 | NFC card type | ISO15693 |
| A8 | Device is stateless | Backend handles session logic (check-in/out pairing) |
| A9 | Public lockout | 5 seconds between presses |
| A10 | Staff lockout | None (multiple actions allowed) |
| A11 | Heartbeat frequency | Once daily |
| A12 | Heartbeat timing | DevEUI-based jitter (distributed across 24 hours) |
| A13 | RTC sync | DeviceTimeReq MAC command after join |
| A14 | Initial "Last Cleaned" | RTC time at first boot |
| A15 | Counter persistence | Survives power cycle and battery replacement |
| A16 | Counter size | 24-bit per button (max 16,777,216) |
| A17 | Reed switch | Hardware present, unused in v1.1 |
| A18 | Indoor only | No weatherproofing requirements |
| A19 | Single LED | All UI feedback via one LED |
| A20 | LoRaWAN Class | Class A |
| A21 | Activation | OTAA |
| A22 | Join retry | 10 attempts per boot, 3 boot cycles then shutdown |

### 8.2 Technical Assumptions

| # | Assumption | Rationale |
|---|------------|-----------|
| T1 | LoRa join will succeed within reasonable attempts | Indoor deployment with gateway coverage |
| T2 | RTC drift is acceptable between daily syncs | ~2 sec/day typical drift |
| T3 | EEPROM has sufficient write endurance | ~1M cycles; counters written per press |
| T4 | Battery life target achievable | 8-10 months with sleep modes |
| T5 | Network provides DeviceTimeAns | Required for RTC sync |

### 8.3 Out of Scope (v1.1)

| Item | Notes |
|------|-------|
| OTA firmware updates | Not supported |
| Mobile app | Not required |
| Complex EPD graphics | Text-only display |
| Outdoor deployment | Indoor only |
| Multi-language EPD | English only |
| Watchdog | Under consideration, not confirmed |

### 8.4 Credential Provisioning (Recommended)

| Component | Approach |
|-----------|----------|
| DevEUI | Derived from nRF52840 FICR (chip unique ID) |
| AppEUI | Compiled into firmware (same for all devices) |
| AppKey | Provisioned per-device via EEPROM (serial/RTT tool) |
