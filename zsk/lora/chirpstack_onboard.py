...
def main(argv: list[str]) -> int:
    ...
    p.add_argument("--server", default="192.168.1.24:8080", help="ChirpStack gRPC host:port (default: %(default)s)")
    p.add_argument("--csv", dest="csv_path", default="eui_registry.csv", help="Path to eui_registry.csv")
    p.add_argument("--application-id", required=True, help="ChirpStack Application ID (UUID)")
    p.add_argument("--device-profile-id", required=True, help="ChirpStack Device Profile ID (UUID)")
    p.add_argument("--token", default="", help="ChirpStack API token (prefer CHIRPSTACK_API_TOKEN env var)")
    ...
    token = (args.token or os.environ.get("CHIRPSTACK_API_TOKEN") or "").strip()
    if not token:
        print(
            "ERROR: missing API token. Set CHIRPSTACK_API_TOKEN or pass --token.",
            file=sys.stderr,
        )
        return 2

    rows = _read_registry(args.csv_path)
