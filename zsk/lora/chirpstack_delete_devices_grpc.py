...
def main(argv: list[str]) -> int:
    ...
    p.add_argument("--server", required=True, help="ChirpStack gRPC host:port")
    p.add_argument("--application-id", required=True, help="Application ID (UUID)")
    p.add_argument("--token", default="", help="API token (prefer CHIRPSTACK_API_TOKEN env var)")
    ...
    if not token:
        print("ERROR: missing API token. Set CHIRPSTACK_API_TOKEN or pass --token.", file=sys.stderr)
        return 2
    ...
    print(f"Listing devices for application {args.application_id}...")
