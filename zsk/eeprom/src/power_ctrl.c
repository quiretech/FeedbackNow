...
int power_ctrl_enable_all(void) {
  int ret;

  LOG_INF("Enabling all power rails...");

  /* Enable 3V3 first (main rail) */
  ret = power_ctrl_set(POWER_EN_3V3, true);
  if (ret != 0) {
    LOG_ERR("Failed to enable 3V3 rail");
    return ret;
  }
  k_msleep(5); /* Allow rail to stabilize */

  /* Enable 1V8 */
  ret = power_ctrl_set(POWER_EN_1V8, true);
  if (ret != 0) {
    LOG_ERR("Failed to enable 1V8 rail");
    return ret;
  }
  k_msleep(5);
  ...
  if (ret != 0) {
    LOG_ERR("Failed to enable 3V3A rail");
    return ret;
  }
  k_msleep(5);
  ...
  if (ret != 0) {
    LOG_ERR("Failed to enable 3V6 rail");
    return ret;
  }
  k_msleep(10); /* Final stabilization delay */
