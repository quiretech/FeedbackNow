...
static volatile uint8_t scan_intent;
static volatile uint8_t scan_button_id;
...
static void nfc_worker_thread(void *a, void *b, void *c) {
  ...
  while (1) {
    ...
    while (k_uptime_get_32() < deadline_ms && !cancel_requested) {
