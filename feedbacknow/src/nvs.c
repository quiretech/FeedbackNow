...
//     }
//   } else {
//     // item not found, let's store keys (generate random 2 x 8byte and 1 x 16
//     // byte, and 2 byte for nonce)
...
//       // 	break;
//     case NVS_DEVNONCE_ID: {
//       uint8_t rand_key[2]; // DevNonce is 2 bytes
//       size_t key_len = nvs_len[id];

//       if (generate_random_key(rand_key, key_len) < 0) {
//         LOG_ERR("Failed to generate random DevNonce");
//         break;
//       }

//       bytes_written = nvs_write(fs, id, rand_key, key_len);
//       if (bytes_written < 0) {
//         LOG_ERR("Failed to write DevNonce to NVS: %d", bytes_written);
//         break;
//       }

//       memcpy(data, rand_key, key_len);
//       LOG_INF("Generated and stored DevNonce: %02X %02X", rand_key[0],
//               rand_key[1]);
//       break;
//     }
//     case NVS_LORAWAN_DEV_EUI_ID:
