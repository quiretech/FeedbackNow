---
title: "FeedbackNow FlexBox v1.2 FRD"
...
| Storage | EEPROM (non-volatile memory); SDHC card slot (out of scope for v1.2; reserved for future) |
| Feedback | 1 LED |
...
| Button 0+1+2 (in Staff Mode) | 3 seconds* | Trigger LoRa join (deliberate / first-time) |
| Button 0+1+2+3 (in Staff Mode) | 10 seconds* | Reboot device |
...
- When disconnected: Only button counters persist in EEPROM (no full event queue)
- LED and EPD feedback operate normally (user sees no difference)
- On rejoin, latest button counts sent via Event 0x07 (Counter Sync) so backend can reconcile
...
| First boot (awaiting join) | "Ready" or similar | Before first deliberate join |
| Boot / Join | "Connecting..." | During LoRa join attempt |
...
- "Cleaning in progress" persists until check-out. Public buttons are not blocked during cleaning; votes are always allowed (C1).
- If custodian does not check out, EPD auto-reverts after 45 minutes* to "Last Cleaned" with current (RTC) timestamp (C2).

**Downlink EPD Updates:**

- Backend can push "Last Cleaned" timestamp via downlink
- EPD downlink updates are processed right after the user vote flow: after "Thanks for your Feedback!" a 5-second* gap is applied, then any pending EPD downlink (e.g. "Last Cleaned") is applied. This batches downlink with the user uplink and avoids overwriting the thank-you message or delaying updates until the next heartbeat.
...
| Check-in Confirmed | 3 blinks | ~2 seconds |
| Check-out Confirmed | 3 blinks | ~2 seconds |
| Registered Vote Confirmed | 3 blinks | ~2 seconds |
...
| Counter Sync (Rejoin) | 0x07 | On successful rejoin after disconnect | Button ID, Counter (per-button uplink; backend reconciles from last known count vs post-rejoin count) |
...
**Disconnected Operation (Counters Only):**

- When network unavailable, only button counters are persisted in EEPROM (no full event payload queue)
- On rejoin: Firmware sends uplinks for each button with latest counter (Event 0x07 Counter Sync: Button ID + Counter)
- Backend reconciles from last known count vs post-rejoin count to infer votes that occurred during disconnect
- Counter-sync uplinks use rate limiting to avoid network congestion

**Network Reconnection:**

- Device attempts rejoin every 1 hour* when disconnected
- On successful join: Counter-sync routine runs (per-button Event 0x07 uplinks with latest counts)
...
- First-join flag ("has_joined_once"; used for deliberate first-join procedure)
- CRC validation on read
- No full event log is stored; only counters and state are persisted

**SDHC Card (out of scope v1.2):**

- SDHC card slot is not used in this version. Planned for a future version: daily log files (button counts, events), counter backup, optional operation without card, Event 0x06 on low space. Retained here for version control.
...
**Note:** Device enters Normal Mode regardless of network state. If disconnected, device operates normally and retries connection every 1 hour*.
...
|------|----------|-------------|
| Network Rejoin | Every 1 hour* (when disconnected) | Attempt OTAA join if disconnected |
...
|-------|-----------|----------|----------|
| **LoRa Join Failure** | No JoinAccept after attempt | Retry with exponential backoff | Enter disconnected mode, retry every 1 hour* |
| **Uplink enqueue/transmission failure (local)** | No local ACK | Queue event for retry | Retry on next uplink opportunity |
...
| **EEPROM CRC Failure** | CRC mismatch on read | Use defaults, set first-boot flag | No backup in v1.2 (SDHC out of scope) |
| **RTC Invalid** | Year &lt; 2026 | Flag for sync | Sync via DeviceTimeReq after join |
...
**Key Principle:** Device NEVER permanently shuts down due to network failure. It remains fully operational in disconnected mode and continuously attempts to reconnect.
...
**Disconnected Mode Capabilities:**

| Feature | Behavior When Disconnected |
|---------|----------------------------|
| Button presses | Accepted normally, LED feedback works |
| EPD updates | Works normally (uses local RTC) |
| NFC check-in/out | Works normally; counters persist |
...
| Parameter | Value |
|-----------|-------|
| Trigger | Successful network rejoin |
| Mechanism | Per-button Event 0x07 uplinks with latest counter |
| Backend | Reconciles from last known count vs post-rejoin count |
| Rate | Rate-limited to avoid congestion |

**Reconnection Strategy:**

| Trigger | Retry Behavior |
|---------|----------------|
| First boot (no has_joined_once) | No join attempts; wait for staff-mode + 0+1+2 deliberate join combo |
| Subsequent boot (disconnected) | 10 attempts* with exponential backoff, then disconnected mode |
| Hourly timer | 3 attempts* with backoff |
| After reconnect | Immediate counter sync (Event 0x07 per button) + RTC sync |
...
| #  | Item | Description | Options | Resolved |
|----|------|-------------|---------|----------|
| C1 | **"Cleaning in progress" button blocking** | Behavior of public buttons while cleaning is in progress | - **A)** Block public buttons during cleaning?<br>- **B)** Allow votes during cleaning? | **B)** Allow votes during cleaning – always allow. |
| C2 | **Custodian no-checkout timeout** | How EPD behaves if custodian does not check out | - **A)** EPD stays "Cleaning in progress" indefinitely<br>- **B)** Auto-revert after X hours<br>- **C)** Auto-revert to previous timestamp<br>- **D)** Auto-revert to current time | **B)** Auto-revert after 45 minutes*; **D)** Auto-revert to current time. |
| C3 | **Downlink EPD latency** | Potential delays if EPD updates are tied to the daily heartbeat | - Updates may be delayed up to 24 hours.<br>- Downlinked updates might not be truly "on demand." | Resolved: EPD downlink applied after user vote (thank you + 5s* gap), batched with user uplink – no 24h delay. |
...
| A4 | Reboot trigger | Enter Staff Mode, then 0+1+2+3 hold 10 seconds* |
...
| A16 | Reed switch | Hardware present, unused in v1.2 |
| A17 | Single LED | All UI feedback via one LED |
| A18 | LoRaWAN Class | Class A |
| A19 | Activation | OTAA |
| A20 | Join / first boot | First boot: no auto-join; deliberate join via Staff Mode + 0+1+2. After has_joined_once: 10 attempts* on boot, then retry every 1 hour* (never shutdown) |
| A21 | Disconnected operation | Device fully functional when disconnected; only counters persisted in EEPROM |
| A22 | Rejoin sync | On rejoin, per-button Event 0x07 (Counter Sync) uplinks with latest counts; backend reconciles from counter deltas |
| A23 | Network resilience | Device never permanently shuts down due to network failure |
| A24 | SDHC | Out of scope v1.2; reserved for future (Event 0x06, logs, backup) |
...
| 1.2 | February 2026 | Reboot: Staff Mode + 0+1+2+3 hold 10s*; deliberate join: Staff Mode + 0+1+2 hold 3s*. EPD downlink after thank you + 5s* gap. LED: check-in/check-out/registered vote 3 blinks. Disconnected model: counters only; Event 0x07 Counter Sync on rejoin. SDHC out of scope v1.2. First-boot no auto-join; has_joined_once flag. Renames: uplink failure (local), RTC &lt; 2026, disconnected mode. Section 7 resolved (C1–C3). |
