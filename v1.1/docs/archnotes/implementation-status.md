...
| has_joined_once / deliberate first join | — | Yes |
...
- **Staff-first:** Join (0+1+2) and Reboot (0+1+2+3) are only handled when already in Staff. From Normal, those combos are ignored and logged as "enter Staff first".
- **Flow:** Enter Staff with 0+1 hold 2s → LED solid, 10s timeout. While in Staff: hold 0+1+2 for 3s → deliberate join (stub), back to Normal; hold 0+1+2+3 for 10s → Reboot (LED 3s then `sys_reboot`). Device Info (0+1+5 hold 3s) from Normal → 30s then back to Normal.
...
4. **Device Info:** Hold 0+1+5 for 3s. Expect `[SMF] Normal -> DeviceInfo (30s timeout)`. Wait 30s: expect `[SMF] DeviceInfo -> Normal (timeout)`.
