...
void BoardGetUniqueId(uint8_t *id) { /* Do not change the default value */ }
...
static void mac_process_notify(void) { LoRaMacProcess(); }
...
static void mcps_indication_handler(McpsIndication_t *mcps_indication) {
  ...
  SYS_SLIST_FOR_EACH_CONTAINER(&dl_callbacks, cb, node) {
    if ((cb->port == LW_RECV_PORT_ANY) || (cb->port == mcps_indication->Port)) {
...
lorawan_join_otaa(const struct lorawan_join_config *join_cfg) {
  MlmeReq_t mlme_req;
  MibRequestConfirm_t mib_req;
...
static LoRaMacStatus_t
lorawan_join_abp(const struct lorawan_join_config *join_cfg) {
  MibRequestConfirm_t mib_req;
...
int lorawan_join(const struct lorawan_join_config *join_cfg) {
  ...
  if (join_cfg->mode == LORAWAN_ACT_OTAA) {
    ...
    if (status != LORAMAC_STATUS_OK) {
      LOG_ERR("OTAA join failed: %s", lorawan_status2str(status));
      ret = lorawan_status2errno(status);
      goto out;
    }
    ...
    if (status != LORAMAC_STATUS_OK) {
      LOG_ERR("ABP join failed: %s", lorawan_status2str(status));
      ret = lorawan_status2errno(status);
      goto out;
    }
  } else {
...
int lorawan_set_class(enum lorawan_class dev_class) {
  ...
  if (mib_req.Param.Class != current_class) {
    ...
    if (status != LORAMAC_STATUS_OK) {
      LOG_ERR("Failed to set device class: %s", lorawan_status2str(status));
      return lorawan_status2errno(status);
    }
  }
...
int lorawan_set_channels_mask(uint16_t *channels_mask,
  ...
  if ((channels_mask == NULL) ||
      (channels_mask_size != region_channels_mask_size)) {
    return -EINVAL;
  }
...
void lorawan_enable_adr(bool enable) {
  ...
  if (enable != atomic_test_bit(lorawan_flags, LORAWAN_FLAG_ADR_ENABLE)) {
    ...
    mib_req.Param.AdrEnable =
        atomic_test_bit(lorawan_flags, LORAWAN_FLAG_ADR_ENABLE);
    LoRaMacMibSetRequestConfirm(&mib_req);
  }
}
...
int lorawan_send(uint8_t port, uint8_t *data, uint8_t len,
  ...
  if (status != LORAMAC_STATUS_OK) {
     ...
     * next try.
     */
    LOG_ERR("LoRaWAN Query Tx Possible Failed: %s", lorawan_status2str(status));
    empty_frame = true;
...
int lorawan_start(void) {
  ...
  status =
      LoRaMacInitialization(&mac_primitives, &mac_callbacks, selected_region);
  if (status != LORAMAC_STATUS_OK) {
    LOG_ERR("LoRaMacInitialization failed: %s", lorawan_status2str(status));
    return -EINVAL;
  }
