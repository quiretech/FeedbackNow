 ...
 * deferred DeviceTimeReq). lorawan_* stays on the LoRa thread
...
/** Daily heartbeat: battery (+ 0x13 snapshot on Plus), then deferred DeviceTime. */
void housekeeping_run(void);

/** DL 0x04 status request: HK telemetry + 6× EVT_COUNTER_SYNC, then DeviceTime. */
