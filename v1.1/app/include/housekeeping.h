 ...
 * housekeeping_run() (battery uplink + link check + jittered counter sync +
 * state snapshot — then deferred DeviceTimeReq like post-join). Downlink
 * DL_CMD_STATUS_REQ
...
/** Heartbeat / telemetry run (rails, HK uplink, link check, jittered counters, snapshot). */
