...
static void smf_thread_fn(void *a, void *b, void *c) {
  ...
  while (1) {
    ...
    if (msg.ev_type == SMF_EVT_DL_REBOOT) {
      ...
      LOG_INF("[SMF] DL reboot (0x07) -> Reboot (LED %u ms then cold boot)",
              (unsigned)REBOOT_LED_MS);
      continue;
    }
...
int smf_post_event(uint8_t ev_type, uint8_t button_id, int64_t timestamp_ms) {
  ...
  if (ret != 0) {
    ...
    LOG_WRN("[SMF] msgq full, dropping ev=%u (ret=%d)", ev_type, ret);
    return -ENOMEM;
  }
  smf_msgq_peak_note();
  return 0;
...
int smf_post_downlink(uint8_t port, uint8_t len, const uint8_t *data) {
  ...
  if (ret != 0) {
    (void)atomic_add(&smf_msgq_drop_count, 1);
    LOG_WRN("[SMF] msgq full, dropping downlink (ret=%d)", ret);
  } else {
    smf_msgq_peak_note();
  }
...
int smf_post_nfc_result(uint8_t ok, uint8_t intent, uint8_t button_id,
  ...
  if (ret != 0) {
    (void)atomic_add(&smf_msgq_drop_count, 1);
    LOG_WRN("[SMF] msgq full, dropping nfc result (ret=%d)", ret);
  } else {
    smf_msgq_peak_note();
  }
  return ret == 0 ? 0 : -ENOMEM;
