...
void app_logic_public_vote(uint8_t button_id)
{
	...
	ret = payload_gen_build_button(payload_button_id, epoch_s, payload, &new_counter);
	if (ret != 0) {
	...
	if (ret == -ENOTCONN) {
		...
		LOG_INF("Queued button uplink: btn=%u ctr=%u ts=%u",
			payload_button_id, new_counter, epoch_s);
	}
}
