...
/* GPIO specs from DT aliases (sw0..sw5 only; 6 buttons) */
...
const struct gpio_dt_spec buttons[NUM_BUTTONS] = {
    ...
    GPIO_DT_SPEC_GET(SW4_NODE, gpios),
    GPIO_DT_SPEC_GET(SW5_NODE, gpios),
};
                       ...
                       uint32_t pins)
{
    for (int i = 0; i < NUM_BUTTONS; i++) {
        if (buttons[i].port == dev && (pins & BIT(buttons[i].pin))) {
            k_timer_start(&debounce_timer[i], K_MSEC(BUTTON_DEBOUNCE_MS),
                          K_NO_WAIT);
        }
    }
}
...
int buttons_init(void)
{
    ...
    for (int i = 0; i < NUM_BUTTONS; i++) {
        ...
        LOG_INF("Button %d on %s pin %d", i, buttons[i].port->name,
                buttons[i].pin);
    }
