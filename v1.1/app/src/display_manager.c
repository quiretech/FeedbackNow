...
#define LV_SYMBOL_LORA_SIGNAL   "\xEF\x82\x9E" // UTF-8 Hex for U+F09E
  ...
  */
  ...
  static void create_lvgl_screens(void) {
    ...
    * space */
    screen_room_turnaround = lv_obj_create(NULL);
    ...
    /* Container for flex layout; must have white background so it doesn't draw
    * black */
    lv_obj_t *cont_room = lv_obj_create(screen_room_turnaround);
    lv_obj_set_size(cont_room, 400, 300);
    lv_obj_center(cont_room);
    lv_obj_set_style_bg_color(cont_room, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(cont_room, LV_OPA_COVER, LV_PART_MAIN);
    lv_obj_set_style_border_width(cont_room, 0, LV_PART_MAIN);
    lv_obj_set_style_pad_all(cont_room, 0, LV_PART_MAIN);
    ...
    next_action_label = lv_label_create(cont_room);
